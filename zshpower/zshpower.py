...
{FG.MAGENTA}Oh My ZSH{NONE} and plugins and activate
                    the {__pkginfo__["name"]} theme.
    {FG.CYAN}activate{NONE} ------ Activate the {__pkginfo__["name"]} theme.
    {FG.CYAN}deactivate{NONE} ---- Deactivate the {__pkginfo__["name"]} theme and \
go back to the default.
    {FG.CYAN}reset{NONE} --------- Reset to default settings.
    {FG.CYAN}uninstall{NONE} ----- Uninstall the package {__pkginfo__["name"]}.
                    {FG.YELLOW}NOTE:{NONE} If you installed {__pkginfo__["name"]} \
with "sudo", use "sudo" with
                    this option as well. {FG.YELLOW}E.g:{NONE} \
{FG.GREEN}sudo {__pkginfo__["pkg_name"]} uninstall{NONE}.
...
[Default: {FG.GREEN}robbyrussell{NONE}].
    ...
    def init_command(self):
        theme_current = utils.read_zshrc(self.zsh_rc)
        if (
            isfile(self.theme_file)
            and isfile(self.config)
            and theme_current[0] == "zshpower"
        ):
            printer("Already configured.", foreground=FG.GREEN)
            exit(0)
